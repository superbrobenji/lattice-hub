...
func TestRejectEnrollment_SendsJoinAckToTargetMac(t *testing.T) {
	...
	if msg.ProtoVersion != 5 {
		t.Errorf("ProtoVersion = %d, want 5", msg.ProtoVersion)
	}
...
func TestApproveEnrollment_NodeIdSet_HasProtoVersion5(t *testing.T) {
	ms := newTestMeshServer(t)
	mockPort := NewMockSerialPort()
	ms.serialComm = NewSerialComm(mockPort)
	...
	if nodeIdMsg.ProtoVersion != 5 {
		t.Errorf("OP_NODE_ID_SET ProtoVersion = %d, want 5", nodeIdMsg.ProtoVersion)
	}
}
...
func TestApproveEnrollment_FullSequence_AllFramesCorrect(t *testing.T) {
	...
	joinAck := decodeWrittenFrame(t, mockPort)
	if joinAck.ProtoVersion != 5 {
		t.Errorf("JOIN_ACK ProtoVersion = %d, want 5", joinAck.ProtoVersion)
	}
	if joinAck.MessageType != 4 {
