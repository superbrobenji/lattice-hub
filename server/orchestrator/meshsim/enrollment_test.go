...
func TestJoinAckApprovesAndStartsHeartbeat(t *testing.T) {
	...
	n := sim.snapshot()[0]
	ack := &mesh.MeshMessage{MessageType: uint32(mesh.MessageTypeJoinAck), TargetMacAddress: []byte{0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x99}, PublicKey: n.PublicKey[:], ProtoVersion: 5}
	go func() {
...
func TestJoinAckEmptyKeyRejectsAndStopsBroadcast(t *testing.T) {
	...
	rej := &mesh.MeshMessage{MessageType: uint32(mesh.MessageTypeJoinAck), TargetMacAddress: []byte{0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x98}, ProtoVersion: 5}
	go func() {
