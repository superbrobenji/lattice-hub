...
func TestHealthFrameLayout(t *testing.T) {
	...
	if led.DataType != int32(mesh.AdapterTypeSerial) || led.ProtoVersion != 4 || led.EpochNum != 0 {
...
func TestOfflineNodeDoesNotAck(t *testing.T) {
	...
	payload := make([]byte, mesh.MaxDataLength)
	payload[0] = byte(mesh.OpLEDSolid)
	cmd := &mesh.MeshMessage{MessageType: uint32(mesh.MessageTypeSerialCmdBroadcast), DataType: int32(mesh.AdapterTypeLED), Data: payload, ProtoVersion: 4}
...
func TestHealthReqMakesAllNodesReport(t *testing.T) {
	sim, orch := newTestSim(t, seededCfg())
	req := &mesh.MeshMessage{MessageType: uint32(mesh.MessageTypeSerialCmdBroadcast), DataType: int32(mesh.AdapterTypeSerial), Data: []byte{byte(mesh.OpHealthReq)}, ProtoVersion: 4}
