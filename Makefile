...
NATIVE_FLAGS := -f server/docker-compose.native.yml
STUB_ENV     := API_KEY=dev ADMIN_KEY=dev

.PHONY: up stub stub-seed native down e2e
...
native: ## Start everything except the orchestrator, which runs natively on the host (macOS; docs/macos_native_dev.md)
	$(COMPOSE) $(NATIVE_FLAGS) up -d
